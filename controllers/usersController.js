...
module.exports = {
    ...
    async findDeliveryMen(req, res, next) {
        try {
            ...
            return res.status(501).json({
                success: false,
                message: 'Error al obtener losencargados'
            });
        }
    },
...
//reports
    async registerReports(req, res, next) {
        try {
            
            const user = req.body;
            const data = await User.createREports(user);
...
async registerWithImageAdmin(req, res, next){
    try {
        const user = JSON.parse(req.body.user);

        console.log(`ddatos enviados del usuario: ${user}`);
        ...
        const data = await User.createAdmin(user);
    ...
    async registerWithImageEncargado(req, res, next){
        try {
            const user = JSON.parse(req.body.user);
            ...
            const data = await User.createEncargado(user);
        
            await Rol.create(data.id, 3); //rol por defecto Cliente
            return res.status(201).json({
                success: true,
                message: 'Registro Exitoso',
                data: data.id
